...
export default function RoomView({ rooms, isFullscreen, setIsFullscreen, onRoomsChange }: RoomViewProps) {
    ...
    const handleRowsChange = (roomId: string, newRows: RoomType["rows"]) => {
        setLocalRooms((prev) => prev.map(r => r.id === roomId ? { ...r, rows: newRows } : r));
    };
        ...
        <div className={isFullscreen ? "fixed inset-0 z-50 bg-background p-4 overflow-y-auto" : "overflow-x-auto overflow-y-auto"}>
            {!isFullscreen && (
                <div className="print:hidden">
                    ...
                    <p className="mb-4">Ajustez la disposition des lames de parquet en les déplaçant.</p>
                </div>
            ...
            <div className={isFullscreen ? "h-[calc(100vh-8rem)]" : ""}>
                {localRooms.map((room) => (
                    <Room room={room} key={room.id} onRowsChange={(rows) => handleRowsChange(room.id, rows)} />
                ))}
            </div>
        </div>
    );

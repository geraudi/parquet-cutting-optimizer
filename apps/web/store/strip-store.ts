...
import { ExportableStoreState, validateImportFile, ImportResult, validateExportableStoreState } from "@web/lib/file-validation";
          ...
          set((state) => {
            try {
              ...
              if (validationErrors.length > 0) {
                throw new Error(`Export validation failed: ${validationErrors.map(e => e.message).join(", ")}`);
              }
